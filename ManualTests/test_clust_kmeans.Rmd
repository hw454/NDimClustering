...
| [4](#test4)   | **4**           | none               | Y                               | N                                                               | standard k-means                | random                            | NA |
| [5](#test5)   | 4               | none               | N                               | N                                                               | standard k-means                | **random from points**            | NA |
...
devtools::install("../ndimclusteringR/", dependencies = TRUE, upgrade = "always")
...
## DBSCAN {#dbscan}
**dbscan** is a density based clustering method. Points are assigned to the cluster if there are within range of another point in the cluster. As a result the clusters can be different shapes and not necessarily balls.
...
### Test 18 {#test18}
Take the case from [test 16](#test16) and introduce the conversion to principal components before clustering. First we will do this without calculating the angles.
...
### Test 20 {#test20}
Now let's investigate the density parameter to **dbscan**. Let's make it smaller and try `point_eps = 0.2`.
