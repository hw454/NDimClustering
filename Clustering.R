...
aim<-trait_axes[1]
stdBeta_df_noEXP_nosus = stdBeta_df_noEXP[-sus_SNP_ind,aim]
eff_df = abs(stdBeta_df_noEXP_nosus)
if ( length(aim)<=1 ){
  eff_dfs=scale(eff_df)
...
# 
# #FIXME when column is one ncol function returns NULL as item is a vector rather than matrix
# if (length(aim)<=1){ncols=1} else{ ncols=ncol(stdBeta_df_noEXP_nosus)}
# trait_h2 = matrix(NA, ncol = ncols, nrow = nClust.AIC)
# for(i in 1:nClust.AIC){
#   row_ind=which(rownames(stdBeta_df_noEXP_nosus) %in% AICclusters_rsid[[i]])
#   aim_ind=which(colnames(stdBeta_df_noEXP_nosus) %in% aim)
#   c_df =  stdBeta_df_noEXP_nosus[row_ind,aim_ind]^2
#   trait_h2[i,] = average(c_df)#colSums(c_df)/nrow(c_df)
# }
