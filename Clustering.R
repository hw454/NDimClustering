...
# Reset the working directory then change to data location
res_dir=getwd()
clus_dir = paste0(res_dir,"/working-example/data/")
setwd(clus_dir)
...
load(paste0(clus_dir,"/QCdata_",EXP_pheno,".Rdata"))
...
exp_dat = fread(paste0(clus_dir,"/sig-clumped-IVs_",EXP_pheno,".csv")) 
out_dat = fread(paste0(clus_dir,"/clumped-IVs_",OUT_pheno,".csv")) 
...
pdf(file = paste0(clus_dir,"/",EXP_pheno,"-",OUT_pheno,"_AICclusters_MR.pdf"), width=8, height = 6)  ###getting corrupt
