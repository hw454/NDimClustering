...
unstd_beta_df <- as.matrix(data.table::fread(paste0(data_dir,"unstdBeta_df.csv")),
                        rownames = 1)
unstd_se_df   <- as.matrix(data.table::fread(paste0(data_dir,"unstdSE_df.csv")),
                        rownames = 1)
...
if (test){
  b_out <- unstd_beta_df[which(colnames(unstd_beta_df) %in% c(out_pheno,exp_pheno))]
  se_out <-unstd_se_df[which(colnames(unstd_se_df)%in% c(out_pheno, exp_pheno))]
  ...
  unstd_beta_df <- cbind(unstdBeta_df[1:num_rows, num_trait0:num_trait1], b_out)
  unstd_se_df <- cbind(unstdSE_df[1:num_rows, num_trait0:num_trait1], se_out)
