...
def get_study(snp_list, id_list):
  ...
  for study_id in id_list:
    ...
    assoc_nsnps=len(assocs)
    if max_nsnp == 0: assocs_out = assocs
    if assoc_snps > max_nsnp:
      max_nsnp = assoc_nsnps
...
# Load GWAS info
ic("GWAS info")
gwas_data = pd.read_csv(input_dir + gwas_cat_csv,
                        sep = ",",
                        header = 0)
ic(gwas_data)
...
outcome_description = trait_df.at[outcome_trait_row,
                                  "Description"].replace(';',' ')
outcome_studies = gwas_df[gwas_df["DISEASE/TRAIT"].str.contains(outcome_description, case = False)]
# Choose the outcome study with the most SNPs
study_ids = outcome_studies.PUBMEDID.unique()
max_nsnp_0 =0
for s_id in study_ids:
  nsnp = len(outcome_studies[outcome_studies.PUBMEDID == s_id])
  if nsnp > max_nsnp_o:
    max_snp_o = nsnp
    max_id_o = s_id
max_nsnp_o = max(outcome_studies["nsnp"])
outcome_study = outcome_studies[outcome_studies.PUBMEDID == max_id_o]
outcome_id = max_id_o
...
max_snp_e = 0
for s_id in study_ids:
  nsnp = len(exposure_studies[exposure_studies.PUBMEDID == s_id])
  if nsnp > max_nsnp_e:
    max_snp_e = nsnp
    max_id_e = s_id
max_nsnp_e = max(exposure_studies["nsnp"])
exposure_study = exposure_studies[exposure_studies.PUBMEDID == max_id_oe]
exposure_id = max_id_e
# Define the initial study by the top hits for the exposure
ic("Get snps ", exposure_id)
